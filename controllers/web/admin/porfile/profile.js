...
module.exports.getInfo = async (req, res, next) => {
    try {
        ...
        // Find the user by ID
        const user = await Manager.findByPk(userId.userID);
        if (!user) {
...
const storage = multer.diskStorage({
    destination: async function (req, file, cb) {
        try {
            ...
            const uploadPath = path.join(__dirname, `../../../../images/profile/Admin/${id}`);
...
module.exports.upload_image = async (req, res, next) => {
    upload(req, res, async function (err) {
        ...
        try {
            const user = await Manager.findByPk(req.userID.userID);
            if (!user) {
                return res.status(404).json({ error: 'User not found' });
            }
            ...
            const fileUrl = `${req.protocol}://${req.get('host')}/images/profile/${req.userID.userID}/${req.file.filename}`;
...
module.exports.editprofile = async (req, res, next) => {
    try {
        ...
        if(user.email!=email){
        const existingEmail = await Manager.findOne({ where: { email: email } });
        if (existingEmail) {
        ...
        const existingPhoneNumber = await Manager.findOne({ where: { phoneNumber: phoneNumber } });
